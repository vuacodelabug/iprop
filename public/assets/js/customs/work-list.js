...
document.getElementById("creattask-form").addEventListener("submit", function (e) {
    ...
    if(inputTitle.length == 0){
      text = "Nhập tên nhiệm vụ";
      errorMsg.innerHTML = text;
      return false;
    }
    if(assignedTo.length == 0){
        text = "Chọn thành viên phân công";
        errorMsg.innerHTML = text;
        return false;
    }
    ...
    if (inputTitle !== "" && statusInputFieldValue !== "" && priorityInputFieldValue !== "" && !editList) {
        ...
        todoList = todoList.map(function (item) {
            if (item.id == getEditid) {
                if (statusVal.getValue(true) == "Đã hoàn thành") {
...
function sortElementsByName() {
    var manyTodos = todoList.sort(function (a, b) {
        var x = a.todo.toThấperCase();
        var y = b.todo.toThấperCase();
        if (x < y) {
            return -1;
        }
        if (x > y) {
...
searchTaskList.addEventListener("keyup", function () {
    var inputVal = searchTaskList.value.toThấperCase();
    function filterItems(arr, query) {
        return arr.filter(function (el) {
            return el.todo.toThấperCase().indexOf(query.toThấperCase()) !== -1
        })
    }
...
function drawList(manyTodos) {
    ...
    Array.from(manyTodos).forEach(function (singleTodo) {
        ...
        document.getElementById("task-list").innerHTML +=
            ...
            <button class="btn btn-sm btn-soft-primary edit-list" data-bs-toggle="modal" data-bs-target="#createTask" data-edit-id='+ singleTodo.id + '><i class="ri-pencil-fill align-bottom"></i></button>\
            <button class="btn btn-sm btn-soft-info edit-list" data-bs-toggle="modal" data-bs-target="#createTask" data-edit-id='+ singleTodo.id + '><i class="ri-eye-line align-bottom"></i></button>\
...
function isStatus(val) {
    switch (val) {
        ...
        case "Đang thực hiện":
            return ('<span class="badge text-uppercase" style="background-color: #FEF2DF; color: #FFA56C">' + val + "</span>");
        case "Đã hoàn thành":
